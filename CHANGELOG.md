...
- The instrument splitter is part of the normal install. Building the app installs it (about 1 GB, into `~/.go-play-in-the-band`; `--no-splitter` leaves it out) and the app starts and stops it, so there is no terminal to leave open. Before, it took a `--stems` flag and a server you ran by hand. The install without Demucs is gone: it made the app try, and fail, to split every recording you loaded.
- Quit button, in the local app only. It stops the app and the instrument splitter at once; before, the only way out was to close the tab and wait two and a half minutes, because the app has no Dock icon. The hosted demo does not show it.
- Fix: `scripts/screenshots.mjs` left Chrome's temporary profile behind on every run (up to 140 MB each). It now waits for Chrome to exit and removes it.
- Practice tools (2026-09-22). Record at any speed: a take made at 75% is stretched back to song time. Recording over a loop cuts the take into passes, each a take of its own, with only the last one playing; "Only this take" and "Delete" on every take row. Saved loops by name, per song. Count-in (one bar of ticks, synthesised on the audio thread). Progressive tempo: start slow, up a step every N passes, the next speed rendered ahead of time. Transpose ±12 semitones (stretch and resample in one pass in `dsp-core`). Chords per bar, estimated from chroma on the bar grid, shown under the whole song and on the loop grid. Tap tempo, time signature (4/4, 3/4, 6/8, 2/4), "Bar 1 here" and 10 ms nudges. Software monitor with the browser's delay shown. Chromatic tuner (YIN in `dsp-core`) with reference tones. Recent songs (files up to 64 MB are kept in the browser) and "My recording" reopen after a reload. Save project as a zip (recording, takes as WAV, loops, tempo, mix, transpose) and load it back; export every part as WAVs in a zip. A second demo, "E waltz", in 3/4. Mac app icon. `docs/demo.gif` at the top of the README, made by `scripts/demo-gif.mjs` with no dependencies; `scripts/smoke.mjs` walks it all in headless Chrome; the Chrome driver is shared in `scripts/lib/chrome.mjs`. Screenshots 10 and 11 added, 01 to 08 retaken.
